...
/**
 * Broadcasted action. Sent only for successful response actions o team/user
 */
export interface ServerBroadcastAction<TType = string, TRequest = object, TResponse = object | undefined> extends ServerRealtimeAction<TType, TResponse> {
    /**
     * Request payload
     */
    requestPayload: TResponse;
    /**
     * Meta information
     */
    meta: {
...
/**
 * Check if given action is ServerBroadcastAction
 */
export function isServerBroadcastAction<TType = string, TRequest = any, TResponse = any>(action: any): action is ServerBroadcastAction<TType, TRequest, TResponse> {
    return (action && typeof action.type !== "undefined" && action.meta && action.meta.broadcast);
}
...
export function isRealtimeRequestAction<TType, TPayload>(action: any): action is RealtimeRequestAction<TType, TPayload> {
    return (isRealtimeAction(action) && action.flowType === REALTIME_ACTION_REQUEST && action.meta.realtime);
}
